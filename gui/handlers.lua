...
minetest.register_on_player_receive_fields(function(player, formname, fields)
    ...
    -- Handle keyboard shortcuts for panning (server-side fallback)
    if fields.key_enter_field then
        ...
        -- Simple WASD-like commands via key field names
        if fields.key_enter_field:find("w") then
            questbook.viewport.pan(player_name, current_chapter, 0, -50)
            questbook.handlers.show_questbook(player_name)
            return true
        elseif fields.key_enter_field:find("s") then
            questbook.viewport.pan(player_name, current_chapter, 0, 50)
            questbook.handlers.show_questbook(player_name)
            return true
        elseif fields.key_enter_field:find("a") then
            questbook.viewport.pan(player_name, current_chapter, -50, 0)
            questbook.handlers.show_questbook(player_name)
            return true
        elseif fields.key_enter_field:find("d") then
            questbook.viewport.pan(player_name, current_chapter, 50, 0)
            questbook.handlers.show_questbook(player_name)
            return true
        end
